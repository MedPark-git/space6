...
function saveNamedBlob(blob,name){const url=URL.createObjectURL(blob);const anchor=document.createElement('a');anchor.href=url;anchor.download=name;document.body.append(anchor);anchor.click();anchor.remove();setTimeout(()=>URL.revokeObjectURL(url),10000)}
async function saveBlob(response){const blob=await response.blob();const disposition=response.headers.get('content-disposition')||'';const encoded=disposition.match(/filename\*=UTF-8''([^;]+)/i);const name=encoded?decodeURIComponent(encoded[1]):'MedPark_회의록.xlsx';saveNamedBlob(blob,name)}
...
// The server has no Korean fonts, so rasterize the server's SVG in the browser, which always has them.
function svgToPng(svg){
 return new Promise((resolve,reject)=>{
  const image=new Image();
  image.onload=()=>{
   const width=image.naturalWidth||image.width,height=image.naturalHeight||image.height;
   const scale=Math.min(2,16000/Math.max(width,height,1));
   const canvas=document.createElement('canvas');canvas.width=Math.round(width*scale);canvas.height=Math.round(height*scale);
   const context=canvas.getContext('2d');context.fillStyle='#fff';context.fillRect(0,0,canvas.width,canvas.height);context.drawImage(image,0,0,canvas.width,canvas.height);
   canvas.toBlob(blob=>blob?resolve(blob):reject(new Error('PNG 이미지 파일을 만들지 못했습니다. 다시 시도해 주세요.')),'image/png');
  };
  image.onerror=()=>reject(new Error('회의록 이미지를 불러오지 못했습니다. 다시 시도해 주세요.'));
  image.src='data:image/svg+xml;charset=utf-8,'+encodeURIComponent(svg);
 });
}
async function downloadImageId(id,title='회의록',date=''){
 try{
  const response=await api('/api/meetings/'+encodeURIComponent(id)+'/image');
  const png=await svgToPng(await response.text());
  const name=String(title).replace(/[^\p{L}\p{N} _-]/gu,'').slice(0,70);
  saveNamedBlob(png,`MedPark_회의록_이미지_${date}_${name}.png`);
 }catch(e){notice(e.message||'PNG 이미지 파일을 만들지 못했습니다. 다시 시도해 주세요.');}
}
async function deleteMeeting(id,title){
 if(busy){notice('분석이 끝난 뒤 삭제해 주세요.');return false;}
 if(!confirm(`'${title}' 회의록을 삭제할까요?\n삭제한 회의록은 되돌릴 수 없습니다.`))return false;
 try{
  await api('/api/meetings/'+encodeURIComponent(id)+'/delete',{method:'POST',body:JSON.stringify({})});
  // If the deleted meeting is open in the editor, keep its text as a new unsaved draft.
  if(meetingId===id){meetingId=null;revision=0;savedStatus='draft';sessionStorage.removeItem('medparkDraftId');$('save-draft').hidden=false;$('write-title').textContent='회의록 작성';}
  if(selectedMeeting&&selectedMeeting.id===id){selectedMeeting=null;$('view-dialog').close();}
  notice('회의록을 삭제했습니다.');
  if($('meeting-rows').children.length<=1&&page>1)page--;
  await loadList();return true;
 }catch(e){notice(e.message);if(e.status===404)await loadList();return false;}
}
...
async function loadList(){const sequence=++listSequence;$('list-error').textContent='';$('list-description').textContent='회의록을 불러오는 중입니다…';const params=new URLSearchParams({category:filterCategory,q:$('search-query').value,from:$('filter-from').value,to:$('filter-to').value,sort:$('sort-order').value,page});if(params.get('from')&&params.get('to')&&params.get('from')>params.get('to')){$('list-error').textContent='조회 시작일이 종료일보다 늦습니다.';return}try{const data=await api('/api/meetings?'+params);if(sequence!==listSequence)return;$('list-total').textContent=data.total;totalPages=Math.max(1,Math.ceil(data.total/data.page_size));$('page-label').textContent=page+' / '+totalPages;$('previous-page').disabled=page<=1;$('next-page').disabled=page>=totalPages;$('list-description').textContent=(filterCategory||'전체')+' 회의록 · '+data.total+'건';$('meeting-rows').replaceChildren();for(const item of data.items){const row=el('tr'),category=el('td'),badge=el('span',item.category,'category-pill');badge.dataset.category=item.category;category.append(badge);const title=el('td'),button=el('button',item.title,'row-title');button.onclick=()=>openMeeting(item.id);title.append(button);const names=item.attendees||[];const attendee=el('td',names.length?names.slice(0,2).join(', ')+(names.length>2?` 외 ${names.length-2}명`:''):'—');attendee.title=names.join(', ');const actions=el('td','','row-actions'),dl=el('button','','download-icon'),png=el('button','','download-icon'),del=el('button','','download-icon');dl.setAttribute('aria-label',item.title+' 엑셀 다운로드');dl.title='엑셀 다운로드';dl.innerHTML='<svg><use href="/static/icons.svg#download"/></svg>';dl.onclick=()=>downloadId(item.id);png.setAttribute('aria-label',item.title+' PNG 이미지 다운로드');png.title='PNG 이미지 다운로드';png.innerHTML='<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="4" width="18" height="16" rx="2"></rect><circle cx="9" cy="10" r="2"></circle><path d="m4 18 5-5 4 4 3-3 4 4"></path></svg>';png.onclick=()=>downloadImageId(item.id,item.title,item.meeting_date);del.setAttribute('aria-label',item.title+' 회의록 삭제');del.title='회의록 삭제';del.style.color='#b42318';del.innerHTML='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16"></path><path d="M9 7V4h6v3"></path><path d="M6 7l1 13h10l1-13"></path><path d="M10 11v6"></path><path d="M14 11v6"></path></svg>';del.onclick=()=>deleteMeeting(item.id,item.title);actions.append(dl,png,del);row.append(category,title,el('td',item.meeting_date),attendee,el('td',item.author||'—'),actions);$('meeting-rows').append(row)}$('list-empty').hidden=data.items.length>0;$('list-empty').querySelector('h3').textContent=params.get('q')||filterCategory||params.get('from')||params.get('to')?'검색된 회의록이 없습니다':'확정된 회의록이 없습니다';$('list-empty').querySelector('p').textContent=params.get('q')||filterCategory||params.get('from')||params.get('to')?'다른 키워드나 조회 조건으로 검색해 주세요.':'회의록을 작성하고 확정하면 이곳에서 볼 수 있습니다.';}catch(e){if(sequence!==listSequence)return;$('list-error').textContent=e.message;$('list-description').textContent='회의록을 불러오지 못했습니다.';$('meeting-rows').replaceChildren();$('list-empty').hidden=true;}}
...
const viewDelete=el('button','삭제','btn secondary danger');viewDelete.type='button';viewDelete.id='view-delete';$('view-download').after(viewDelete);viewDelete.onclick=()=>{if(selectedMeeting)deleteMeeting(selectedMeeting.id,selectedMeeting.title)};
